...
# The graphical keyboard layout, in the form libinput and X11 clients read.
#
# Niri does not consult this — its layout is set in its own configuration — but
# anything else that ever runs on this machine does, including a different
# compositor swapped in later. Written from the same XKB_LAYOUT and XKB_VARIANT
# so the two cannot describe different keyboards.
x11_keyboard_configuration() {
    printf '# Generated by arch-framework-installer.\n'
...
" || return 1
    write_target_file /etc/X11/xorg.conf.d/00-keyboard.conf "$(x11_keyboard_configuration)" || return 1
